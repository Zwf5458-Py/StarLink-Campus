...
-- 海星智联智慧校园数据库初始化脚本
-- 版本: 1.0.0
-- 说明: 包含组织架构、幼儿/教职工档案、考勤打卡、晨检健康、安防巡检访客、家园共育表
-- ====================================================================
...
-- 1. 园区与班级表
CREATE TABLE IF NOT EXISTS `kg_campus_info` (
  `id` BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '园区ID',
  `campus_name` VARCHAR(100) NOT NULL COMMENT '园区名称',
  `address` VARCHAR(255) COMMENT '园区地址',
  `contact_phone` VARCHAR(20) COMMENT '联系电话',
  `status` TINYINT DEFAULT 1 COMMENT '状态: 0禁用, 1启用',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB COMMENT='园区信息表';

CREATE TABLE IF NOT EXISTS `kg_class` (
  `id` BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '班级ID',
  `campus_id` BIGINT NOT NULL COMMENT '园区ID',
  `grade_level` VARCHAR(20) NOT NULL COMMENT '年级(小班/中班/大班)',
  `class_name` VARCHAR(50) NOT NULL COMMENT '班级名称',
  `head_teacher_id` BIGINT COMMENT '班主任ID',
  `student_count` INT DEFAULT 0 COMMENT '班级人数',
  `room_number` VARCHAR(30) COMMENT '教室编号/班牌号',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (`campus_id`) REFERENCES `kg_campus_info`(`id`) ON DELETE CASCADE
) ENGINE=InnoDB COMMENT='班级表';

-- 2. 幼儿与教职工档案表
CREATE TABLE IF NOT EXISTS `kg_student` (
  `id` BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '幼儿ID',
  `class_id` BIGINT NOT NULL COMMENT '所属班级ID',
  `name` VARCHAR(50) NOT NULL COMMENT '姓名',
  `gender` TINYINT COMMENT '性别: 1男, 2女',
  `birthday` DATE COMMENT '出生日期',
  `avatar_url` VARCHAR(255) COMMENT '照片URL',
  `ic_card_no` VARCHAR(50) UNIQUE COMMENT '物理IC卡/ID卡卡号',
  `face_feature_id` VARCHAR(100) COMMENT 'CompreFace/ArcFace 人脸特征ID',
  `allergies` TEXT COMMENT '过敏源信息(Json格式或逗号分割)',
  `guardian_name` VARCHAR(50) COMMENT '主要家长姓名',
  `guardian_phone` VARCHAR(20) COMMENT '家长电话',
  `status` TINYINT DEFAULT 1 COMMENT '状态: 1在读, 2毕业, 3退园',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (`class_id`) REFERENCES `kg_class`(`id`)
) ENGINE=InnoDB COMMENT='幼儿档案表';

CREATE TABLE IF NOT EXISTS `kg_staff` (
  `id` BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '教职工ID',
  `name` VARCHAR(50) NOT NULL COMMENT '姓名',
  `phone` VARCHAR(20) UNIQUE NOT NULL COMMENT '手机号',
  `role_type` VARCHAR(30) COMMENT '角色(教师/保育员/保健医/安检门卫/园长)',
  `avatar_url` VARCHAR(255) COMMENT '头像URL',
  `ic_card_no` VARCHAR(50) UNIQUE COMMENT 'IC卡号',
  `face_feature_id` VARCHAR(100) COMMENT '人脸特征ID',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP
) ENGINE=InnoDB COMMENT='教职工档案表';

-- 3. 考勤系统表 (幼儿 & 教职工)
CREATE TABLE IF NOT EXISTS `kg_student_attendance` (
  `id` BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '主键ID',
  `student_id` BIGINT NOT NULL COMMENT '幼儿ID',
  `class_id` BIGINT NOT NULL COMMENT '班级ID',
  `attendance_date` DATE NOT NULL COMMENT '考勤日期',
  `check_in_time` DATETIME COMMENT '入园时间',
  `check_in_type` VARCHAR(20) COMMENT '入园方式: FACE(刷脸), CARD(刷卡), MANUAL(补卡)',
  `check_in_temperature` DECIMAL(4,1) COMMENT '入园体温(℃)',
  `check_out_time` DATETIME COMMENT '离园时间',
  `check_out_type` VARCHAR(20) COMMENT '离园方式',
  `status` VARCHAR(20) DEFAULT 'NORMAL' COMMENT '考勤状态: NORMAL正常, LATE迟到, ABSENT缺勤, LEAVE请假',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (`student_id`) REFERENCES `kg_student`(`id`)
) ENGINE=InnoDB COMMENT='幼儿每日考勤记录';

-- 4. 晨检与健康保健表
CREATE TABLE IF NOT EXISTS `kg_morning_check` (
  `id` BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '晨检记录ID',
  `student_id` BIGINT NOT NULL COMMENT '幼儿ID',
  `check_date` DATE NOT NULL COMMENT '晨检日期',
  `temperature` DECIMAL(4,1) NOT NULL COMMENT '检测体温(℃)',
  `is_fever` TINYINT DEFAULT 0 COMMENT '是否发热: 0否, 1是',
  `health_tags` VARCHAR(255) COMMENT '晨检异常标签(咳嗽,手足疱疹,红眼等)',
  `inspector_id` BIGINT COMMENT '晨检教医师ID',
  `remark` VARCHAR(255) COMMENT '备注说明',
  `create_time` DATETIME DEFAULT CURRENT_TIMESTAMP,
  FOREIGN KEY (`student_id`) REFERENCES `kg_student`(`id`)
) ENGINE=InnoDB COMMENT='晨检健康记录表';

-- 5. 智慧班牌资源配置表
CREATE TABLE IF NOT EXISTS `kg_class_board_config` (
  `id` BIGINT AUTO_INCREMENT PRIMARY KEY COMMENT '配置ID',
  `class_id` BIGINT UNIQUE NOT NULL COMMENT '班级ID',
  `board_mode` VARCHAR(30) DEFAULT 'NORMAL' COMMENT '班牌当前模式: NORMAL(平时), CHECK_IN(考勤打卡), EMERGENCY(紧急通知)',
  `slogan` VARCHAR(255) DEFAULT '快乐成长，健康每一步！' COMMENT '班级口号',
  `update_time` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  FOREIGN KEY (`class_id`) REFERENCES `kg_class`(`id`)
) ENGINE=InnoDB COMMENT='智慧班牌配置表';

-- 初始测试数据插入
INSERT INTO `kg_campus_info` (`id`, `campus_name`, `address`, `contact_phone`) VALUES
(1, '海星幼儿园 (总园区)', '深圳市南山区科技园路88号', '0755-88889999');

INSERT INTO `kg_class` (`id`, `campus_id`, `grade_level`, `class_name`, `student_count`, `room_number`) VALUES
(1, 1, '大班', '大(1)班 - 葵花班', 28, 'A101'),
(2, 1, '中班', '中(2)班 - 满天星班', 25, 'B202');

INSERT INTO `kg_student` (`id`, `class_id`, `name`, `gender`, `birthday`, `ic_card_no`, `allergies`) VALUES
(101, 1, '张小明', 1, '2020-05-12', 'CARD_1001', '花生,芒果'),
(102, 1, '李思思', 2, '2020-08-20', 'CARD_1002', '无'),
(103, 1, '王豆豆', 1, '2020-03-15', 'CARD_1003', '牛奶');

INSERT INTO `kg_class_board_config` (`class_id`, `board_mode`, `slogan`) VALUES
(1, 'NORMAL', '探索世界，梦想起航！');
-- ----------------------------
-- 1、部门表
-- ----------------------------
